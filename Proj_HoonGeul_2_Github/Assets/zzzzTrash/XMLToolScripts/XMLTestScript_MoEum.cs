...
//자음개수 : 19
//모음개수 : 21
public class XMLTestScript_MoEum : MonoBehaviour
{
    ...
    */
    ...
    /// for count
    int[] chosungCountArr = new int[21 * 21];


    Dictionary<string, int> CountTbl = new Dictionary<string, int>();
    string[] countKey_arr = new string[21 * 21];
    int[] countValue_arr = new int[21 * 21];
    string[] choQuest_arr= new string[21 * 21];
    ...
    void LoadXml()
    {
        TextAsset textAsset = (TextAsset)Resources.Load("2umjul_Hanja"); // WordDictionary.xml 파일은 Resources안에 있음. 
        Debug.Log(textAsset);
        ...
        {
            AllNodes = AllNodes + WordToValue("1", node.SelectSingleNode("Key").InnerText) + "\n"; // foreach문을 돌면서 모든 <Word>를 불러옴.
        }
        Debug.Log(AllNodes);
    }
    /*
    ...
    string WordToValue(string wordType /*단어 어원*/ ,string inputWord)
    {
        ...
        {
            output += Divide_Hangul(word_array[i]).ToString();
        }
    ...
    public int Divide_Hangul(char c_input)
    {
        ...
        */
        ...
        int wordValue=0;
        char []jung_temp = m_Jung.ToCharArray();
